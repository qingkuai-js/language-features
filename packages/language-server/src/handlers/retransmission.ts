...
export async function attachRetransmissionHandlers() {
    ...
    connection.onRequest(LS_HANDLERS.Retransmission, async (params: RetransmissionParams) => {
    ...
    connection.onNotification(LS_HANDLERS.Retransmission, async (params: RetransmissionParams) => {
        if (tpicConnectedPromise.state === "pending") {
            await tpicConnectedPromise
        }
