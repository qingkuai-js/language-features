...
describe("Emmet functions:", () => {
    ...
    test("whether emmet is avaliable in any text content section.", async () => {
        ...
        await openContentAsTextDocument(formatSourceCode(`
                <div>
                    <input>span
            `));
    ...
    test("whether emmet is avaliable for special attribute(dynamic/reference attribute, directive name or event name).", async () => {
        ...
        await doComplete(2, 31).then(completions => {
            ...
            expect(completions?.[0].textEdit?.newText).toBe("<div #for={item, index in arr}>${0}</div>");
        });
        await doComplete(3, 42).then(completions => {
            expect(completions?.[0].documentation).toBe("<span !class={|} &value={|} #for={|} @keyup|stop|once={|}>|</span>");
            expect(completions?.[0].textEdit?.newText).toBe("<span !class={${1}} &value={${2}} #for={${3}} @keyup|stop|once={${4}}>${0}</span>");
        });
        await doComplete(4, 44).then(completions => {
            expect(completions?.[0].documentation).toBe("<div @click={()=>{console.log(expression)}}>|</div>");
            expect(completions?.[0].textEdit?.newText).toBe("<div @click={()=>{console.log(expression)}}>${0}</div>");
        });
    });
});
...
describe("Html Attribute name completions:", () => {
    const assertSuggestCommand = (item) => {
...
export async function doComplete(line, character, triggerChar) {
